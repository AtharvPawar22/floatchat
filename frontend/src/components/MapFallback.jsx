...
const MapFallback = ({ selectedFloat, onFloatSelect }) => {
  return (
    <div className="w-full h-full bg-gradient-to-br from-blue-900 to-slate-900 rounded-lg relative overflow-hidden">
      ...
      <div className="absolute top-[20%] left-[15%] w-32 h-24 bg-green-700/30 rounded-xl transform rotate-12 pointer-events-none" />
      <div className="absolute top-[40%] right-[20%] w-28 h-20 bg-green-700/30 rounded-lg transform -rotate-6 pointer-events-none" />
      <div className="absolute bottom-[25%] left-[25%] w-36 h-16 bg-green-700/30 rounded-2xl pointer-events-none" />
      ...
      {mockFloats.map((float, index) => {
        ...
        return (
          <button
            ...
            className={`absolute w-4 h-4 rounded-full border-2 transition-all duration-200 hover:scale-150 z-10 ${
      ...
      {/* Title overlay */}
      <div className="absolute top-4 left-4 text-cyan-400 text-sm font-medium pointer-events-none">
        2D Ocean Map View
      </div>
    </div>
  );
